...
stdin, stdout, stderr = c.exec_command(
    f"{PY} -c 'import torch;print(\"torch\",torch.cuda.is_available());import evaluate;print(\"eval OK\")' 2>&1", timeout=10)
print("Verify:", stdout.read().decode().strip()[:200], stderr.read().decode().strip()[:200])

# Launch on GPU2 with eval!
...
print(f"Launched train_v3_{ts}.log GPU2 eval=200")

time.sleep(130)

stdin, stdout, stderr = c.exec_command(
    f"tail -5 /home/jiaqigu/mrrate_hidnet/outputs/report_gen/train_v3_{ts}.log 2>/dev/null", timeout=10)
