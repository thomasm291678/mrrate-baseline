...
# Check if code exists on farm05
s, o, e = c.exec_command("ls /home/jiaqigu/mrrate_hidnet/scripts/train.py 2>/dev/null && echo 'EXISTS' || echo 'MISSING'", timeout=10)
print("Code:", o.read().decode().strip())

# Check GPU0 memory (farm05 has 100GB cards)
...
# GPU0 has 18GB used out of 98GB - enough room
# Launch on GPU0
...
print(f"farm05 GPU0 launched: train_farm05_{ts}.log")

time.sleep(150)

s, o, e = c.exec_command(
    f"tail -5 /home/jiaqigu/mrrate_hidnet/outputs/report_gen/train_farm05_{ts}.log; "
    f"nvidia-smi --query-gpu=index,memory.used,utilization.gpu --format=csv,noheader | grep '^0,'",
    timeout=10)
print(o.read().decode())
