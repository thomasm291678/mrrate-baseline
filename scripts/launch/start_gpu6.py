...
cmd = (
    ...
    f"> outputs/report_gen/train_gpu6_{ts}.log 2>&1 &"
)
c.exec_command(f"nohup bash -c '{cmd}' &", timeout=5)
print(f"GPU6 launched: train_gpu6_{ts}.log")

time.sleep(150)

s, o, e = c.exec_command(
    f"tail -5 /home/jiaqigu/mrrate_hidnet/outputs/report_gen/train_gpu6_{ts}.log; "
    f"nvidia-smi --query-gpu=index,memory.used,utilization.gpu --format=csv,noheader | grep '^6,'",
    timeout=10)
print(o.read().decode())
