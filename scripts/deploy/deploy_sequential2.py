...
sftp = c.open_sftp()
sftp.putfo(BytesIO(script.encode()), "/tmp/v4_sequential.sh")
sftp.close()
...
c.exec_command(
    ...
    timeout=5)
print(f"Launched: v4_sequential_{ts}.log")
print("T1(batch=4) -> T2(batch=4) -> Flair(batch=4) | Each 3 epochs, eff_batch=8")

time.sleep(240)

# Find the T1 log (it's created inside train_v4.py)
...
if t1_log:
    ...
    # maybe still loading, check sequential log
    s, o, e = c.exec_command(
        f"tail -20 /home/jiaqigu/mrrate_hidnet/outputs/report_gen/v4_sequential_{ts}.log 2>/dev/null",
        timeout=10)
    print("\n=== Sequential log ===")
    print(o.read().decode())
