...
c.exec_command(
    "for s in $(ipcs -s | grep jiaqigu | awk '{print $2}'); do ipcrm -s $s 2>/dev/null; done; true",
    timeout=10)
...
sftp.close()
print("Uploaded train_v4.py")

# Launch with nohup, phase=uniformer, batch=2, ga=3
...
print(f"Config: batch=2, ga=3 (eff=6), phase=uniformer, projector+LLM frozen")
