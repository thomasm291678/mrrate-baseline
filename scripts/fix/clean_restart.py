...
# Check disk
stdin, o, e = c.exec_command("df -h /home/jiaqigu/mrrate_hidnet/outputs/report_gen/", timeout=10)
print("Disk before:", o.read().decode().strip().split("\n")[-1])

# Clean old logs and keep last 3 step checkpoints, delete corrupted one
c.exec_command(
    "cd /home/jiaqigu/mrrate_hidnet/outputs/report_gen && "
    "rm -f step_001800.pt && "  # corrupted 257MB only
    "rm -f train_eval_*.log train_farm02_*.log train_202*.log watch_*.log "
    "fix*.log repair.log startup.log watchdog*.log monitor.log 2>/dev/null; "
    "true", timeout=10)

# Keep only most recent 2 step checkpoints
stdin, o, e = c.exec_command(
    "ls -t /home/jiaqigu/mrrate_hidnet/outputs/report_gen/step_*.pt | tail -n +3 | xargs rm -f 2>/dev/null; "
    "ls -lh /home/jiaqigu/mrrate_hidnet/outputs/report_gen/step_*.pt 2>/dev/null; echo '---';"
    "df -h /home/jiaqigu/mrrate_hidnet/outputs/report_gen/ | tail -1", timeout=10)
print(o.read().decode())
...
time.sleep(130)
stdin, o, e = c.exec_command(f"tail -8 /home/jiaqigu/mrrate_hidnet/outputs/report_gen/{log_name}", timeout=10)
print(o.read().decode())
