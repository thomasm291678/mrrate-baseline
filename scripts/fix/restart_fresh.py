...
# Check zombie workers
stdin, o, e = c.exec_command("ps -u jiaqigu | grep python | grep -v grep", timeout=10)
print("Python procs:", o.read().decode().strip())

# Force clean
c.exec_command("fuser -k /dev/nvidia2 2>/dev/null; pkill -9 -f train.py; true", timeout=5)
time.sleep(10)

stdin, o, e = c.exec_command("nvidia-smi --query-gpu=index,memory.used --format=csv,noheader | grep '^2,'", timeout=10)
print("GPU2:", o.read().decode().strip())

# Restart from zero (no resume — the step_001600.pt resume seems broken)
...
stdin, o, e = c.exec_command(f"tail -5 /home/jiaqigu/mrrate_hidnet/outputs/report_gen/train_v3_{ts}.log", timeout=10)
print(o.read().decode())
