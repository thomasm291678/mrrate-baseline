...
print(f"Restarted: train_v5_encoder_{ts}.log (workers=4)")

time.sleep(120)

s, o, e = c.exec_command(
    f"tail -5 /home/jiaqigu/mrrate_hidnet/outputs/report_gen/train_v5_encoder_{ts}.log 2>/dev/null",
    timeout=10)
print("\n" + o.read().decode())

# Check GPU 3 times rapidly
for i in range(3):
    s, o, e = c.exec_command("nvidia-smi --query-gpu=index,utilization.gpu --format=csv,noheader | grep '^3,'", timeout=10)
    print(f"GPU3 util [{i}]: {o.read().decode().strip()}")
