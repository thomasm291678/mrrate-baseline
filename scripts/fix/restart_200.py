...
# Upload updated train.py
sftp = c.open_sftp()
sftp.put(r"C:\Users\HP\Documents\5555\train.py", "/home/jiaqigu/mrrate_hidnet/scripts/train.py")
sftp.close()
print("Uploaded train.py (save_interval=200)")

# Kill current training
c.exec_command("pkill -f 'train_v3_' 2>/dev/null; true", timeout=5)
time.sleep(5)

# Verify free
stdin, stdout, stderr = c.exec_command("nvidia-smi --query-gpu=index,memory.used --format=csv,noheader | grep '^2,'", timeout=10)
print(f"GPU2 before launch: {stdout.read().decode().strip()}")

# Launch with save_interval=200
...
print(f"Launched train_v3_{ts}.log (save every 200 steps)")

time.sleep(130)

stdin, stdout, stderr = c.exec_command(
    f"tail -8 /home/jiaqigu/mrrate_hidnet/outputs/report_gen/train_v3_{ts}.log 2>/dev/null", timeout=10)
