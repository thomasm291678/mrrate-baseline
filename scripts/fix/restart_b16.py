...
cmd = (
    ...
    f"--batch_size 16 --ga_steps 1 --epochs 2 --num_workers 2 "
    f"--lr 3e-4 --wd 1e-4 --grad_clip 1.0 "
    f"--grid 2 --base_ch 32 "
    f"--log_dir outputs/report_gen "
    f"--save_interval 1000 --auto_save_interval 200 --log_interval 5 "
    f"--auto_resume "
    f">> outputs/report_gen/train_v5_encoder_{ts}.log 2>&1 &"
)
c.exec_command(f"nohup bash -c '{cmd}' &", timeout=5)
print(f"V5 encoder: train_v5_encoder_{ts}.log (batch=16, 2x speed)")

time.sleep(180)

s, o, e = c.exec_command(
    f"tail -8 /home/jiaqigu/mrrate_hidnet/outputs/report_gen/train_v5_encoder_{ts}.log 2>/dev/null",
    timeout=10)
print("\n" + o.read().decode())
