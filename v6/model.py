...
if __name__ == "__main__":
    ...
    out = enc(x)
    print(f"MRRCNN: {tuple(x.shape)} -> {tuple(out.shape)}  tokens={enc.n_tokens}  params={sum(p.numel() for p in enc.parameters()):,}")

    has = torch.tensor([True, True])
    dummy = torch.randn(2, 1, 128, 128, 128)
    model = MultiModalEncoder(llm_dim=512, grid=2, base_ch=32)
    vt = model(dummy, dummy, dummy, has, has, has)
    total = sum(p.numel() for p in model.parameters())
    print(f"MultiModalEncoder: {tuple(vt.shape)}  tokens_per_mod={model.n_tokens_per_mod}  total={model.total_tokens}  params={total:,}")
